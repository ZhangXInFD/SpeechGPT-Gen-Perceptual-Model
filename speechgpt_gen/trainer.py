        ...
        for epoch in range(self.epochs):
            ...
            for batch in self.dl:
                ...
                with self.accelerator.accumulate(self.model):
                    ...
                    if self.is_main:
                        accum_log(logs, {'loss': loss.item() / self.accelerator.gradient_accumulation_steps, 'acc': acc.item() / self.accelerator.gradient_accumulation_steps})
                    ...
                    if self.accelerator.sync_gradients:    
                        ...
                        if self.is_main and not (steps % self.log_steps):
                            self.print(f"Epoch {epoch} -- Step {steps}: loss: {logs['loss']:0.3f}\tacc:{logs['acc']:0.3f}\ttime cost per step:{(time.time() - tic) / self.log_steps:0.3f}s")
                            tic = time.time()
